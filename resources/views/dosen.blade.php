...
<div class="container">
    ...
    <form action="{{ route('dosen.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            <label for="jabatan_akademik_id" class="form-label">Jabatan Akademik</label>
            <select name="jabatan_akademik_id" id="jabatan_akademik_id" class="form-select">
                <option value="">-- Pilih Jabatan Akademik --</option>
                @foreach($jabatanAkademik as $j)
                    <option value="{{ $j->id }}" {{ old('jabatan_akademik_id') == $j->id ? 'selected' : '' }}>{{ $j->nama_jabatan_akademik }}</option>
                @endforeach
            </select>
        </div>

        {{-- Input Golongan, bisa ajax juga --}}
        <div class="mb-3">
    ...
    $('#jabatan_akademik').on('change', function () {
        ...
        if (jabatan_akademikID) {
            $.get('/get-golongan/' + jabatan_akademikID, function (data) {
                ...
                $.each(data, function (key, value) {
                    $('#golongan').append('<option value="' + key + '">' + value + '</option>');
                });
            });
        } else {
